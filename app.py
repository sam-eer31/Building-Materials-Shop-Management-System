        ...
        if db_uri.startswith('mysql+pymysql://'):
            ...
            if len(uri_parts) >= 2:
                ...
                if '@' in connection_part:
                    ...
                    username, password, host, port = '', '', connection_part, 3306
            ...
            print(f"⚠️ Database creation not implemented for URI format: {db_uri}")
            
    except Exception as e:
        print(f"⚠️ Error creating database: {e}")
        print("Please ensure the database server is running and credentials are correct")
...
def insert_sample_data():
    """Insert sample data into the database if it doesn't exist"""
    try:
        ...
        sample_products = [
            Product(name='Portland Cement', price=350.00, stock_quantity=100, unit='bag'),
            Product(name='Red Bricks', price=12.00, stock_quantity=5000, unit='piece'),
            Product(name='River Sand', price=1200.00, stock_quantity=20, unit='truck'),
            Product(name='Steel Rods 12mm', price=450.00, stock_quantity=200, unit='piece'),
            Product(name='Steel Rods 16mm', price=650.00, stock_quantity=150, unit='piece'),
            Product(name='Crushed Stone', price=800.00, stock_quantity=15, unit='truck'),
            Product(name='White Cement', price=450.00, stock_quantity=50, unit='bag'),
            Product(name='Concrete Blocks', price=25.00, stock_quantity=1000, unit='piece')
        ]
        ...
        sample_customers = [
            Customer(name='ABC Construction', phone='0123456789', address='123 Main Street, Downtown'),
            Customer(name='XYZ Builders', phone='0987654321', address='456 Industrial Area, City'),
            Customer(name='City Developers', phone='0555666777', address='789 Business District, Town')
        ]
...
def recreate_database():
    ...
    try:
        ...
        return jsonify({'message': 'Database recreated successfully! Default admin user created (username: admin, password: admin123) and sample data inserted.'})
    except Exception as e:
...
def insert_sample_data_route():
    ...
    try:
        ...
        return jsonify({'error': f'Failed to insert sample data: {str(e)}'}), 500
...
def create_admin_route():
    ...
    try:
        create_default_admin()
        return jsonify({'message': 'Default admin user created successfully! (username: admin, password: admin123)'})
    except Exception as e:
